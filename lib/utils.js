...
var successHandler = function (response) {
  if (_.isArray(response)) {
    ...
    try {
      ...
      log.tag('successHandler').error({message: 'caught an error', error: error});
      throw error;
    }
  }
  else {
...
var getInitialAccessToken = function (tokenService, clientId, clientSecret, redirectUri, code, domainPrefix, state) {
  ...
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - token_service: ' + tokenService);
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - client Id: ' + clientId);
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - client Secret: ' + (clientSecret) ? '<secret>' : '<missing>');
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - redirect Uri: ' + redirectUri);
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - code: ' + code);
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - domain_prefix: ' + domainPrefix);
  log.tag('getInitialAccessToken').debug('getInitialAccessToken - state: ' + state);
  ...
  return request.post(options) // TODO: why to the catch blocks eat errors and not send back any resolve/reject ?
    .then(successHandler)
    .catch(RateLimitingError, function (e) {
      log.tag('getInitialAccessToken').error('A RateLimitingError error like "429 Too Many Requests" happened: '
        + e.statusCode + ' ' + e.response.body + '\n'
        + JSON.stringify(e.response.headers, null, 2));
    })
    .catch(ClientError, function (e) {
      log.tag('getInitialAccessToken').error('A ClientError happened: '
        + e.statusCode + ' ' + e.response.body + '\n'
        /*+ JSON.stringify(e.response.headers,null,2)
    ...
    .catch(function (e) {
      log.tag('getInitialAccessToken').error({message: 'An unexpected error occurred', error: e});
    });
};

var refreshAccessToken = function (tokenService, clientId, clientSecret, refreshToken, domainPrefix) {
  // TODO: tweak winston logs to prefix method signature (like breadcrumbs) when logging?
  log.tag('refreshAccessToken').debug('token service: ' + tokenService);
  log.tag('refreshAccessToken').debug('client Id: ' + clientId);
  log.tag('refreshAccessToken').debug('client Secret: ' + (clientSecret) ? '<secret>' : '<missing>');
  log.tag('refreshAccessToken').debug('refresh token: ' + (refreshToken) ? '<secret>' : '<missing>');
  log.tag('refreshAccessToken').debug('domain prefix: ' + domainPrefix);
  ...
  return request.post(options)
    ...
    .catch(function (e) {
      log.tag('refreshAccessToken').error({message: 'An unexpected error occurred', error: e});
    });
};
...
var retryWhenConnectionFails = function (args, connectionInfo, callback, retryCounter) {
  if (retryCounter<3) {
    ...
    log.tag('retryWhenConnectionFails').debug('retry after: ' + retryAfter + ' ms');

    return Promise.delay(retryAfter)
      .then(function () {
        log.tag('retryWhenConnectionFails').debug(retryAfter + ' ms have passed...');
 ...
 */
var retryWhenRateLimited = function (args, connectionInfo, callback, retryCounter) {
  if (retryCounter<3) {
    var retryAfter = 5 * 60 * 1000; // by default Vend will never block for more than 5 minutes
    /**
     * Vend no more gives out retry-after info in error body
     */
...
var sendRequest = function (options, args, connectionInfo, callback, retryCounter) {
  ...
  return request(options)
    ...
    .catch(ConnectError, function (e) {
      log.error({message: 'A ConnectError happened', error: e});
    ...
    .catch(RateLimitingError, function (e) {
      log.error({
        message: 'A RateLimitingError error like "429 Too Many Requests" happened',
        error: {
          error: e //logging the whole error to try and find out if Vend gives out a Retry-After header like it says in docs
        }
        //+ JSON.stringify(e.response.headers,null,2)
      });
      /**
       * By passing the code to find retry-after coz Vend no more gives out this info in err body
       */
    ...
    .catch(function (e) {
      log.tag('sendRequest').error({message: 'vend.js - sendRequest - An unexpected error occurred', error: e});
      throw e; // TODO: throw unknown errors but reject well known errors?
    });
};
