  ...
  describe('when a refreshToken is available', function () {
    ...
    describe.only('with sales API', function(){
      ...
      describe('this will create a sale with all the relevant data', function () {
        ...
        /**
         * Conclusion - a GREAT deal of data massaging is required in order to prepare a sale body for
         *              sending an update if the json fetched from a 2.x endpoint is the starting point
         *   1. `line_items` may have to be turned into `register_sale_products`
         *   2. `payments` may have to be turned into `register_sale_payments`
         *   3. `receipt_number` may have to be turned into `invoice_number`
         *   4. `taxes[0].amount` may have to be turned into `taxes[0].tax`
         *   5. etc. etc. there is just too many differences in the json schema to valdiate and too many permutations to test
         *   6. posting the 2.x fetched body as-is destroys a lot of the data
         */
        it('re-posting a fetchedByIdSale from 2.x endpoints "as-is" will destroy a lot of the data', function() {
          var args = vendSdk.args.sales.fetchById();
          args.apiId.value = registerSale.id;
          return vendSdk.sales.fetchById(args, getConnectionInfo())
            .then(function (response1) {
              var fetchedByIdSale1 = response1.data;
              // fetch it again for comparison
              return vendSdk.sales.fetchById(args, getConnectionInfo())
                .then(function (response2) {
                  var fetchedByIdSale2 = response2.data;

                  expect(fetchedByIdSale1).to.deep.equal(fetchedByIdSale2);

                  // 6. posting the 2.x fetched body as-is destroys a lot of the data
                  return vendSdk.sales.create(response1.data, getConnectionInfo())
                    .then(function (response3) {
                      args.apiId.value = response3.register_sale.id;
                      return vendSdk.sales.fetchById(args, getConnectionInfo())
                        .then(function (response4) {
                          var fetchedByIdSale3 = response4.data;

                          //expect(fetchedByIdSale1).to.deep.equal(fetchedByIdSale3); // use this line if you want to see the DIFF
                          expect(fetchedByIdSale1).to.not.deep.equal(fetchedByIdSale3); // use the above line if you want to see the DIFF
                        });
                    });
                });
            });
